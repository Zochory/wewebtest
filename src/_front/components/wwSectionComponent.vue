...
<template>
    ...
    >
        <a :id="anchorId" href="#" class="hash-anchor"></a>
        <!-- BACKGROUND VIDEO -->
...
export default {
    ...
    setup(props) {
        ...
        const wwIsInStretchedSection = computed(() => {
            return content['_ww-layout_alignItems'] === 'stretch';
        });
        ...
        const {
            ...
            configuration,
            name: sectionName,
         } = useComponentData({ type: 'section', uid: props.uid, currentStates });

        /* wwFront:start */
        const isConditionalRenderingResponsive = useIsConditionalRenderingResponsive({
            type: 'section',
            uid: props.uid,
        });
        /* wwFront:end */
        ...
        const isRendering = computed(() => {
            ...
            if (isConditionalRenderingResponsive.value && window.__WW_IS_PRERENDER__) {
                return false;
            }
        ...
        return {
            ...
            anchorId: computed(() => {
                const { sectionTitle } = wwLib.$store.getters['websiteData/getSection'](props.uid) || {};
