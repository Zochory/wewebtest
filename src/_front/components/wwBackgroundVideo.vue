...
export default {
    ...
    computed: {
        poster() {
            if (this.video.value.poster) {
                 /* wwFront:start */
                // eslint-disable-next-line no-unreachable
                return `${this.video.value.poster}`;
                /* wwFront:end */
            } else {
                return '';
            }
        ...
        attributes() {
            const attributes = {
                src: this.video.value.url,
                poster: this.poster,
            };

            if (this.video.value.loop) attributes.loop = true;
            if (this.video.value.preload) attributes.preload = true;

            if (this.video.value.size === 'contain') {
                attributes.style = {
                    'max-height': '100%',
                    'max-width': '100%',
                };
