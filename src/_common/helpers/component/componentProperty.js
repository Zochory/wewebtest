...
import { sendEventToSegment } from '@/helpers/trackingHelper.js';
...
export function setProperty(
    { propertyConfiguration, value, allComponents, path },
    screen,
    state,
 ) {
    propertyConfiguration = unref(propertyConfiguration);
    allComponents = unref(allComponents);
    if (propertyConfiguration.editorOnly) {
        for (const element of allComponents) {
    ...
    if (propertyConfiguration.responsive) {
        screen = screen || wwLib.$store.getters['front/getScreenSize'];
    }
    let media = screen || 'default';
    if (propertyConfiguration.states) {
    ...
    for (const element of allComponents) {
        ...
        sendEventToSegment({
            event: 'COMPONENT_VALUE_UPDATED',
            path: unref(path),
            value,
            isWwObject: element.type === 'element',
            breakpoint: screen || wwLib.$store.getters['front/getScreenSize'],
            state: state || wwLib.$store.getters['manager/getSidebarElementState'],
            uid: element.type === 'element',
        });
    }
}

export function resetProperties(allComponents, properties, state = 'default') {
    properties.forEach(property => {
        const screens = property.configuration.responsive ? allScreenMedia.value : ['default'];
...
export function resetPropertiesDefinedByClass(allComponents, properties, state, classId, subClassId) {
    properties.forEach(property => {
        const screens = property.configuration.responsive ? allScreenMedia.value : ['default'];
        const classData = wwLib.$store.getters['libraries/getClasses'][classId];
        const data =
            (subClassId ? classData && classData.subClasses && classData.subClasses[subClassId] : classData) || {};
        const firstDefinedIndex = screens.findIndex(screen => {
            const propertyClassValue = getComponentRawProperty({
                data,
            ...
            return propertyClassValue !== undefined;
        });
        if (firstDefinedIndex === -1) return;
