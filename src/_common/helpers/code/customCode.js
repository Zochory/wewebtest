...
export function sortData(data, sort, context, event, args) {
    ...
    data.sort((a, b) => {
        for (const elem of computedSort) {
            let result = 0;
            let type =
                a[elem.key] === null || a[elem.key] === undefined
                    ? b[elem.key] === null
                        ? 'undefined'
                        : typeof b[elem.key]
                    : typeof a[elem.key];
            switch (type) {
                ...
                case 'number':
                    result =
                        (a[elem.key] || 0) > (b[elem.key] || 0) ? 1 : (a[elem.key] || 0) < (b[elem.key] || 0) ? -1 : 0;
                    break;
                case 'object':
...
export function getValue(rawValue, context, { event, recursive = true, defaultUndefined, args } = {}) {
    ...
    if (rawValue.__wwtype === 'f') {
        ...
        value = rawValue.map(raw => getValue(raw, context, { event, args }));
