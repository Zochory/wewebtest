...
import { executeComponentAction } from '@/_common/use/useActions';
        ...
        switch (action.type) {
            ...
            case 'reset-variables': {
                ...
                for (const varId of action.varsId || []) {
                    if (!varId) continue;
                    const variable = wwLib.$store.getters['data/getVariable'](varId);
                    if (variable) {
            ...
            case 'upload-file': {
                ...
                const fileVariable = isVariable
                    ? isInternalVariable
                        ...
                        : wwLib.$store.getters['data/getComponentVariables'][action.varId]
                    : null;
                ...
                if (isVariable) {
                    ...
                    if (!actionValue) throw new Error('No file selected.');
                } else {
                ...
                const element = isVariable
                    ? wwLib.$store.getters['websiteData/getWwObject'](fileVariable.componentUid) || {}
                    : null;
                    ...
                    await axios.put(data.signedRequest, file, {
                        ...
                        onUploadProgress: data => {
                            ...
                            if (isInternalVariable) {
                                context?.component?.methods?.updateVariable(
                                    `${fileVariable.componentUid}-progress`,
                                    progress + ((data.loaded / data.total) * 100) / files.length
                                );
                            } else {
            ...
            case 'file-encode-base64': {
                ...
                if (typeof action.file === 'string') {
                    ...
                    if (innerComponentVariables[action.file]) {
                        ...
                        file = wwLib.$store.getters['data/getComponentVariables'][action.file]?.value;
                    }
                } else {
                ...
                logActionInformation('info', 'Generate Base64 encoded file', { preview: result });
                break;
            }
            ...
            default: {
                ...
                try {
                    const promise = wwLib.wwPlugins[plugin.name][currentAction.code](args, wwUtils);
